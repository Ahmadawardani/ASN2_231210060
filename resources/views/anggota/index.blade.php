            ...
            <div class="col-lg-10">
                <div class="table-responsive">
                    <table class="table table-hover table-bordered bg-white shadow-sm rounded">
                        ...
                        <tbody>
                            @forelse ($anggota as $id => $item)
                                <tr>
                                    <td>{{ $item['nim'] }}</td>
                                    <td>{{ $item['nama'] }}</td>
                                    <td>{{ $item['jabatan'] }}</td>
                                    <td>
                                        <a href="{{ route('anggota.show', $id) }}" class="btn btn-sm btn-outline-info">Detail</a>
                                        <a href="{{ route('anggota.edit', $id) }}" class="btn btn-sm btn-outline-warning">Edit</a>
                                        <a href="{{ route('anggota.confirmDelete', $id) }}" class="btn btn-sm btn-outline-danger">Hapus</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
